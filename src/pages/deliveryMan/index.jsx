...
import { MdEmail, MdSchedule, MdUpdate } from "react-icons/md";
...
export default function Profail() {
  ...
  const fetchRider = useCallback(async () => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        if (response?.data?.data?.data.length > 0 && !detailsId) {
  ...
  const fetchRiderDetails = async () => {
    setLoading(true);
    try {
      const response = await NetworkServices.Rider.show(detailsId);
      if (response?.status === 200) {
...
<div className="flex border-b border-gray-200 w-1/2 mt-4">
      ...
      <div className="lg:flex  gap-4">
              ...
              <div className="absolute -bottom-10 bg-white rounded-full border-4 border-primary p-2">
                {riderDetails?.profile_image ? (
                  <img
                    ...
                    alt={riderDetails?.name || "Profile"}
                    className="w-20 h-20 rounded-full object-cover shadow-2xl p- transform scale-105 z-10"
                  />
                ) : (
            ...
            <div className="pt-12 px-2 pb-6 ">
              ...
              <div className="text-left space-y-2 text-sm text-gray-600">
                ...
                <p>
                  ...
                  {riderDetails?.date_of_birth
                    ? riderDetails.date_of_birth.split("T")[0]
                    : "N/A"}
                </p>
              </div>
            </div>
        ...
        <div className=" bg-white rounded-lg  mt-3 w-full">
          ...
          <div className="bg-white rounded overflow-y-auto mb-10 shadow">
            {loading ? (
              <ListSkeleton />
            ) : (
              <DataTable
